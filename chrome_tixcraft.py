        ...
        try:
            ...
            if tixcraft_family:
                ...
                if len(tixcraft_sid) > 1:
                    ...
                    driver.add_cookie(
                        {"name": "SID", "value": tixcraft_sid, "path": "/", "secure": True})
...
def ticketmaster_date_auto_select(driver, url, config_dict, domain_name):
    ...
    try:
        area_list = driver.find_elements(
            By.CSS_SELECTOR, '#list-view > div > div.event-listing > div.accordion-wrapper > div')
    except Exception as exc:
        print("find #gameList fail")
            ...
            if row_is_enabled:
                try:
                    ...
                    if "price" in zone_info[row]:
                        row_text += " " + \
                            zone_info[row]["price"][0]["ticketPrice"]
                except Exception as exc:
                    if show_debug_message:
...
def ticketmaster_area_auto_select(driver, config_dict, zone_info):
    ...
    if len(area_keyword) > 0:
        ...
        try:
            ...
            area_keyword_array = []
        for area_keyword_item in area_keyword_array:
            is_need_refresh, matched_blocks = get_ticketmaster_target_area(
                config_dict, area_keyword_item, zone_info)
            if not is_need_refresh:
                break
            else:
                print("is_need_refresh for keyword:", area_keyword_item)
                    ...
                    if previous_answer != ocr_answer:
                        ...
                        if True:
                            ...
                            if not Captcha_Browser is None:
                                ...
                                if new_captcha_url != "":
                                    tixcraft_change_captcha(
                                        driver, new_captcha_url)  # 更改CAPTCHA圖
    else:
...
def tixcraft_ticket_main_agree(driver, config_dict):
    ...
    for i in range(3):
        is_finish_checkbox_click = check_checkbox(
            driver, By.CSS_SELECTOR, '#TicketForm_agree')
...
def tixcraft_ticket_main(driver, config_dict, ocr, Captcha_Browser, domain_name):
    ...
    is_ticket_number_assigned, select_obj = tixcraft_assign_ticket_number(
        driver, config_dict)
...
def kktix_confirm_order_button(driver):
    ...
    try:
        # method #3 wait
        next_step_button = wait.until(EC.element_to_be_clickable(
            (By.CSS_SELECTOR, 'div.form-actions a.btn-primary')))
        if not next_step_button is None:
            if next_step_button.is_enabled():
                next_step_button.click()
                ret = True
...
def kktix_events_press_next_button(driver):
    is_button_clicked = press_button(
        driver, By.CSS_SELECTOR, '.tickets > a.btn-point')
...
def kktix_travel_price_list(driver, config_dict, kktix_area_auto_select_mode, kktix_area_keyword):
    ...
    try:
        ticket_price_list = driver.find_elements(
            By.CSS_SELECTOR, 'div.display-table-row')
    except Exception as exc:
        ticket_price_list = None
    ...
    if price_list_count > 0:
        ...
        kktix_area_keyword_1_and = util.format_keyword_string(
            kktix_area_keyword_1_and)
        ...
        for row in ticket_price_list:
            ...
            if len(row_text) > 0:
                if ticket_number > 1:
                    ...
                    if ' danger' in row_html and ' Left ' in row_html:
                        ...
                        if len(tmp_array) > 0:
                            tmp_ticket_count = tmp_array[len(
                                tmp_array)-1].strip()
                            if tmp_ticket_count.isdigit():
                                if show_debug_message:
                                    print("found ticket left:",
                                          tmp_ticket_count)
                                ticket_count = int(tmp_ticket_count)
                    ...
                    if ticket_count < ticket_number:
                        ...
                        if show_debug_message:
                            print("found ticket left:", tmp_ticket_count,
                                  ",but target ticket:", ticket_number)
                        row_text = ""
            ...
            if len(row_text) > 0:
                ...
                try:
                    ticket_price_input = row.find_element(
                ...
                if not ticket_price_input is None:
                    ...
                    try:
                        current_ticket_number = str(
                            ticket_price_input.get_attribute('value')).strip()
...
def kktix_assign_ticket_number(driver, config_dict, kktix_area_keyword):
    ...
    matched_blocks = None
    is_dom_ready = True
    is_dom_ready, is_ticket_number_assigned, matched_blocks = kktix_travel_price_list(
        driver, config_dict, auto_select_mode, kktix_area_keyword)
    ...
    if not target_area is None:
        ...
        try:
            current_ticket_number = str(
                target_area.get_attribute('value')).strip()
        except Exception as exc:
            pass
...
def kktix_check_agree_checkbox(driver, config_dict):
    ...
    if is_dom_ready:
        is_finish_checkbox_click = check_checkbox(
            driver, By.CSS_SELECTOR, '#person_agree_terms')
...
def kktix_double_check_all_text_value(driver, ticket_number):
    ...
    try:
        ...
        ticket_price_input_list = driver.find_elements(
            By.CSS_SELECTOR, my_css_selector)
    except Exception as exc:
        pass
...
def kktix_reg_captcha(driver, config_dict, fail_list, registrationsNewApp_div):
    ...
    question_text = get_text_by_selector(
        driver, 'div.custom-captcha-inner p', 'innerText')
    if len(question_text) > 0:
        ...
        if len(answer_list) == 0:
            if config_dict["advanced"]["auto_guess_options"]:
                answer_list = util.get_answer_list_from_question_string(
                    registrationsNewApp_div, question_text)
...
def kktix_reg_new_main(driver, config_dict, fail_list, played_sound_ticket):
    ...
    try:
        registrationsNewApp_div = driver.find_element(
            By.CSS_SELECTOR, '#registrationsNewApp')
    except Exception as exc:
        pass
    ...
    if not registrationsNewApp_div is None:
        ...
        if len(area_keyword) > 0:
            ...
            for area_keyword_item in area_keyword_array:
                ...
                is_dom_ready, is_ticket_number_assigned, is_need_refresh_tmp = kktix_assign_ticket_number(
                    driver, config_dict, area_keyword_item)
            ...
            # empty keyword, match all.
            is_dom_ready, is_ticket_number_assigned, is_need_refresh = kktix_assign_ticket_number(
                driver, config_dict, "")
        ...
        if is_dom_ready:
            ...
            if is_ticket_number_assigned:
                ...
                fail_list, is_question_popup = kktix_reg_captcha(
                    driver, config_dict, fail_list, registrationsNewApp_div)

                # single option question
                if not is_question_popup:
                    # no captcha text popup, goto next page.
                    control_text = get_text_by_selector(
                    ...
                    if len(control_text) > 0:
                        ...
                        try:
                            input_text_element = driver.find_element(
                                By.CSS_SELECTOR, input_text_css)
                        except Exception as exc:
                            # print(exc)
                            pass
                        if input_text_element is None:
                            ...
                            try:
                                radio_element = driver.find_element(
                                ...
                                if radio_element:
                                    ...
                                    joined_element = driver.find_element(
                                        By.CSS_SELECTOR, joined_button_css)
                                    if joined_element:
                                        control_text = ""
                                        print("member joined")
                            except Exception as exc:
            ...
            if len(date_keyword) == 0 and len(area_keyword_item) == 0:
                ...
                for row in formated_area_list:
                    ...
                    try:
                        ...
                        td_date = row.find_element(
                            By.CSS_SELECTOR, my_css_selector)
                        if not td_date is None:
                            # print("date:", td_date.text)
                            date_html_text = util.format_keyword_string(
                                td_date.text)

                        my_css_selector = "td:nth-child(2)"
                        td_area = row.find_element(
                            By.CSS_SELECTOR, my_css_selector)
                        if not td_area is None:
                            # print("area:", td_area.text)
                            area_html_text = util.format_keyword_string(
                                td_area.text)
                    ...
                    if len(row_text) > 0:
                        ...
                        if len(area_keyword_item) > 0:
                            ...
                            area_keyword_array = area_keyword_item.split(' ')
                            for area_keyword in area_keyword_array:
                                area_keyword = util.format_keyword_string(
                                    area_keyword)
                                if not area_keyword in row_text:
                                    is_match_area = False
                                    break
...
def fami_activity(driver):
    ...
    try:
        fami_start_to_buy_button = driver.find_element(
            By.CSS_SELECTOR, '#buyWaiting')
    except Exception as exc:
        pass
...
def fami_date_auto_select(driver, config_dict, last_activity_url):
    ...
    if not target_area is None:
        ...
        for i in range(3):
            ...
            try:
                my_css_selector = "button"
                el_btn = target_area.find_element(
                    By.CSS_SELECTOR, my_css_selector)
            except Exception as exc:
                pass
...
def fami_date_to_area(driver, config_dict, last_activity_url):
    ...
    if len(area_keyword) > 0:
        ...
        for area_keyword_item in area_keyword_array:
            is_need_refresh, is_price_assign_by_bot = fami_area_auto_select(
                driver, config_dict, area_keyword_item)
            if not is_need_refresh:
                break
            else:
        ...
        # empty keyword, match all.
        is_need_refresh, is_price_assign_by_bot = fami_area_auto_select(
            driver, config_dict, area_keyword)
...
def fami_home_auto_select(driver, config_dict, last_activity_url):
    ...
    if ticket_el is None:
        ...
        is_date_assign_by_bot = fami_date_auto_select(
            driver, config_dict, last_activity_url)
        is_price_assign_by_bot = fami_date_to_area(
            driver, config_dict, last_activity_url)
    ...
    if is_ticket_number_assigned:
        ...
        try:
            ...
            fami_assign_site_button = driver.find_element(
                By.CSS_SELECTOR, my_css_selector)
        except Exception as exc:
            pass
        ...
        if not fami_assign_site_button is None:
            ...
            if is_visible:
                try:
                    ...
                    try:
                        driver.execute_script(
                            "arguments[0].click();", fami_assign_site_button)
                    except Exception as exc:
                        pass
    ...
    if not is_select_box_visible:
        ...
        if len(area_keyword) > 0:
            ...
            for area_keyword_item in area_keyword_array:
                matched_blocks = get_fami_target_area(
                    driver, config_dict, area_keyword_item)
                if not matched_blocks is None:
                    break
                else:
        ...
        if area_list_count > 0:
            ...
            for row in area_list:
                ...
                try:
                    ...
                    if not el_btn is None:
                        button_class_string = str(
                            el_btn.get_attribute('class'))
            ...
            if len(date_keyword) == 0:
                ...
                for row in formated_area_list:
                    ...
                    if len(row_text) > 0:
                        if show_debug_message:
                            print("row_text:", row_text)
                        is_match_area = util.is_row_match_keyword(
                            date_keyword, row_text)
                        if is_match_area:
                            matched_blocks.append(row)
...
def urbtix_purchase_ticket(driver, config_dict):
    ...
    if show_debug_message:
        print("date_keyword:", date_keyword)
    is_date_assign_by_bot = urbtix_date_auto_select(
        driver, date_auto_select_mode, date_keyword, auto_reload_coming_soon_page_enable)
...
def urbtix_ticket_number_auto_select(driver, config_dict):
    ...
    ticket_price_input = None
    try:
        ticket_price_input = driver.find_element(
            By.CSS_SELECTOR, "input.ticket-count")
    except Exception as exc:
        pass
    ...
    if is_ticket_number_assigned:
        ...
        try:
            ...
            el_ticket_count = driver.find_element(
                By.CSS_SELECTOR, my_css_selector)
...
def urbtix_uncheck_adjacent_seat(driver, config_dict):
    ...
    try:
        ...
        el_checkbox_icon = driver.find_element(
            By.CSS_SELECTOR, my_css_selector)
...
def urbtix_performance(driver, config_dict):
    ...
    if len(area_keyword) > 0:
        ...
        for area_keyword_item in area_keyword_array:
            ...
            is_need_refresh, is_price_assign_by_bot = urbtix_area_auto_select(
                driver, config_dict, area_keyword_item)
        ...
        if area_list_count > 0:
            ...
            for row in area_list:
                ...
                try:
                    ...
                    span_price_limited = row.find_element(
                        By.CSS_SELECTOR, my_css_selector)
                    if not span_price_limited is None:
                        span_i18n_string = str(
                            span_price_limited.get_attribute('data-i18n'))
...
def cityline_purchase_button_press(driver, config_dict):
    ...
    is_date_assign_by_bot = cityline_date_auto_select(
        driver, date_auto_select_mode, date_keyword, auto_reload_coming_soon_page_enable)
...
def cityline_performance(driver, config_dict):
    ...
    if auto_fill_ticket_number:
        ...
        if len(area_keyword) > 0:
            ...
            for area_keyword_item in area_keyword_array:
                is_need_refresh, is_price_assign_by_bot = cityline_area_auto_select(
                    driver, config_dict, area_keyword_item)
                if not is_need_refresh:
                    break
                else:
            ...
            for row in area_list:
                ...
                if len(row_text) > 0:
                    ...
                    if is_seat_remaining_checking:
                        try:
                            area_seat_el = row.find_element(
                                By.CSS_SELECTOR, 'td.action')
                            if not area_seat_el is None:
                                ...
                                if seat_text.isdigit():
                                    ...
                                    if seat_int < config_dict["ticket_number"]:
                                        ...
                                        if show_debug_message:
                                            print(
                                                "skip not enought ticket number area at row_text:", row_text)
                                        row_text = ""
                        except Exception as exc:
                            if show_debug_message:
                                print(exc)
...
def ibon_performance(driver, config_dict):
    ...
    if len(area_keyword) > 0:
        ...
        for area_keyword_item in area_keyword_array:
            is_need_refresh, is_price_assign_by_bot = ibon_area_auto_select(
                driver, config_dict, area_keyword_item)
            if not is_need_refresh:
                break
            else:
...
def ibon_purchase_button_press(driver):
    is_button_clicked = press_button(
        driver, By.CSS_SELECTOR, '#ticket-wrap > a.btn')
    return is_button_clicked
...
def cityline_login(driver, account, password):
    is_email_sent = assign_text(
        driver, By.CSS_SELECTOR, 'input[type="text"]', account, submit=True)

    # press "click here" use password to login.
    if is_email_sent:
        is_click_here_pressed = press_button(
            driver, By.CSS_SELECTOR, '.otp-box > ul > li:nth-child(3) > a')

    is_password_sent = False
    if is_email_sent:
        is_password_sent = assign_text(
            driver, By.CSS_SELECTOR, 'div > input[type="password"]', password, submit=True)
...
def kham_login(driver, account, password):
    ...
    if is_password_sent:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, 'div.memberContent > p > a > button.red')
...
def ticket_login(driver, account, password):
    ...
    if is_password_sent:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, 'input[value="登入"]')
...
def udn_login(driver, account, password):
    ...
    if not is_logined:
        ...
        if is_email_sent:
            is_password_sent = assign_text(
                driver, By.CSS_SELECTOR, '#password', password, submit=False)
    return is_password_sent
...
def ticketmaster_get_ticketPriceList(driver, config_dict):
    ...
    try:
        ...
        div_mapContainer = driver.find_element(
            By.CSS_SELECTOR, my_css_selector)
    except Exception as exc:
        if show_debug_message:
            print('fail to find my_css_selector:', my_css_selector)
    ...
    if not div_mapContainer is None:
        ...
        try:
            ...
            div_loadingmap = driver.find_element(
                By.CSS_SELECTOR, my_css_selector)
        ...
        if not is_loading:
            try:
                ...
                table_select = driver.find_element(
                    By.CSS_SELECTOR, my_css_selector)
            except Exception as exc:
                if show_debug_message:
                    print('fail to find my_css_selector:', my_css_selector)
...
def ticketmaster_assign_ticket_number(driver, config_dict):
    ...
    if not table_select is None:
        ...
        try:
            ...
            form_select = table_select.find_element(
                By.CSS_SELECTOR, my_css_selector)
        except Exception as exc:
            if show_debug_message:
                print('my_css_selector:', my_css_selector)
    ...
    if not is_ticket_number_assigned:
        ...
        if is_ticket_number_assigned:
            is_button_clicked = press_button(
                driver, By.CSS_SELECTOR, '#autoMode')
...
def tixcraft_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    if '/artist/' in url and 'ticketmaster.com' in url:
        ...
        if len(url.split('/')) == 6:
            if config_dict["date_auto_select"]["enable"]:
                domain_name = url.split('/')[2]
                is_date_selected = ticketmaster_date_auto_select(
                    driver, url, config_dict, domain_name)
    ...
    if '/ticket/area/' in url:
        ...
        if config_dict["area_auto_select"]["enable"]:
            if not 'ticketmaster' in domain_name:
                ...
                tixcraft_dict["fail_promo_list"] = ticketmaster_promo(
                    driver, config_dict, tixcraft_dict["fail_promo_list"])
    ...
    if '/ticket/check-captcha/' in url:
        domain_name = url.split('/')[2]
        ticketmaster_captcha(driver, config_dict, ocr,
    ...
    if '/ticket/checkout' in url:
        if not tixcraft_dict["start_time"] is None:
            if not tixcraft_dict["done_time"] is None:
                bot_elapsed_time = tixcraft_dict["done_time"] - \
                    tixcraft_dict["start_time"]
                if tixcraft_dict["elapsed_time"] != bot_elapsed_time:
                    print("bot elapsed time:",
                          "{:.3f}".format(bot_elapsed_time))
...
def kktix_paused_main(driver, url, config_dict):
    ...
    if not is_url_contain_sign_in:
        if '/registrations/new' in url:
            ...
            recaptcha_div = None
            try:
                recaptcha_div = driver.find_element(
                    By.CSS_SELECTOR, '.event-captcha-info')
            except Exception as exc:
                pass
            ...
            if not recaptcha_div is None:
                ...
                class_name = 'disabled'
                remove_attribute_tag_by_selector(
                    driver, select_query, class_name)
...
def kktix_main(driver, url, config_dict):
    ...
    if not is_url_contain_sign_in:
        if '/registrations/new' in url:
            ...
            is_dom_ready = False
            is_finish_checkbox_click = False
            is_dom_ready, is_finish_checkbox_click = kktix_check_agree_checkbox(
                driver, config_dict)
            ...
            if not is_dom_ready:
                ...
                if config_dict["kktix"]["auto_fill_ticket_number"]:
                    kktix_dict["fail_list"], kktix_dict["played_sound_ticket"] = kktix_reg_new_main(
                        driver, config_dict, kktix_dict["fail_list"], kktix_dict["played_sound_ticket"])
    ...
    if is_kktix_got_ticket:
        if not kktix_dict["start_time"] is None:
            if not kktix_dict["done_time"] is None:
                bot_elapsed_time = kktix_dict["done_time"] - \
                    kktix_dict["start_time"]
            ...
            if not kktix_dict["is_popup_checkout"]:
                ...
                if len(kktix_account) > 0:
                    ...
                    threading.Thread(target=util.launch_maxbot, args=(
                        script_name, "", url, kktix_account, kktix_password, "", "false",)).start()
                ...
                if is_event_page:
                    ...
                    if confirm_clicked:
                        domain_name = url.split('/')[2]
                        checkout_url = "https://%s/account/orders" % (
                            domain_name)
                        print("搶票成功, 請前往該帳號訂單查看: %s" % (checkout_url))
                        webbrowser.open_new(checkout_url)
...
def fami_login(driver, account, password):
    ...
    if is_email_sent:
        is_password_sent = assign_text(
            driver, By.CSS_SELECTOR, '#usr_pwd', password, submit=True)
    return is_password_sent
...
def famiticket_main(driver, url, config_dict):
    ...
    if '/Home/Activity/Info/' in url:
        ...
        fami_dict["fail_list"] = fami_verify(
            driver, config_dict, fami_dict["fail_list"])
    else:
        fami_dict["fail_list"] = []

    if '/Sales/Home/Index/' in url:
        if config_dict["date_auto_select"]["enable"]:
            is_date_assign_by_bot = fami_home_auto_select(
                driver, config_dict, fami_dict["last_activity"])
...
def urbtix_performance_confirm_dialog_popup(driver):
    ...
    el_div = None
    try:
        el_div = driver.find_element(
            By.CSS_SELECTOR, 'div.notification-confirm-btn > div.button-text')
    except Exception as exc:
        ...
        if seq > 0:
            if len(option_text_string) > 1:
                if seq <= len(option_text_string):
                    ...
                    if direction == "right":
                        question_answer_char = option_text_string[len(
                            option_text_string)-seq:len(option_text_string)-seq+1]
            ...
            if len(tmp_seq) > 0:
                ...
                if len(count_target_string) > 0:
                    if count_target_string.isdigit():
                        ...
                        count_target = util.chinese_numeric_to_int(
                            count_target_string)
...
def urbtix_auto_survey(driver, config_dict):
    ...
    questions_div = None
    try:
        questions_div = driver.find_elements(
            By.CSS_SELECTOR, 'div.modal-content > div.content > div.questions > div.question-item')
    except Exception as exc:
        # print("find modal-dialog fail")
        # print(exc)
        pass
    ...
    if not questions_div is None:
        ...
        try:
            for each_question_div in questions_div:
                each_question_ask_div = each_question_div.find_element(
                    By.CSS_SELECTOR, 'div.titles > div')
                if not each_question_ask_div is None:
                    ...
                    question_answer_char, question_direction = get_urbtix_survey_answer_by_question(
                        question_text)
                    each_option_items_div = each_question_div.find_elements(
                        By.CSS_SELECTOR, 'div.options > div.option-item')
                    if not each_option_items_div is None:
                        question_answered = False
                        for each_option_div in each_option_items_div:
                            option_content_div = each_option_div.find_element(
                                By.CSS_SELECTOR, 'div.content-list')
                            if not option_content_div is None:
                                ...
                                option_content_div_text = util.full2half(
                                    option_content_div_text)
                                ...
                                if question_direction in ['left', 'right']:
                                    for answer_item in util.synonym_dict(question_answer_char):
                                        if answer_item in option_content_div_text:
                                            is_radio_clicked = press_button(
                                                each_option_div, By.CSS_SELECTOR, 'div.radio-wrapper')
                                            if is_radio_clicked:
                                                if show_debug_message:
                                                    print("fill answer:",
                                                          answer_item)
    ...
    if is_radio_clicked and fill_question_count >= 2:
        ...
        try:
            questions_remain_div = driver.find_element(
                By.CSS_SELECTOR, 'div.surplus-questions-number')
...
def urbtix_main(driver, url, config_dict):
    ...
    waiting_for_access_url = [
        '/session/landing-timer/', 'msg.urbtix.hk', 'busy.urbtix.hk']
    ...
    if '.hk/member-login' in url:
        ...
        if urbtix_password == "":
            urbtix_password = util.decryptMe(
    ...
    if is_performace_page:
        if config_dict["area_auto_select"]["enable"]:
            is_confirm_dialog_popup = urbtix_performance_confirm_dialog_popup(
                driver)
...
def cityline_cookie_accept(driver):
    is_btn_click = press_button(
        driver, By.CSS_SELECTOR, '.cookieWrapper_closeBtn')
...
def cityline_main(driver, url, config_dict):
    ...
    if 'cityline.com/Login.html' in url:
        ...
        if cityline_password == "":
            cityline_password = util.decryptMe(
...
def ibon_captcha(driver, config_dict, ocr, Captcha_Browser, model_name):
    ...
    last_url, is_quit_bot = get_current_url(driver)
    for redo_ocr in range(19):
        is_need_redo_ocr, previous_answer, is_form_sumbited = ibon_auto_ocr(
            driver, config_dict, ocr, away_from_keyboard_enable, previous_answer, Captcha_Browser, ocr_captcha_image_source, model_name)
...
def ibon_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    home_url_list = ['https://ticket.ibon.com.tw/', 'https://ticket.ibon.com.tw/index/entertainment'
                     ]
    for each_url in home_url_list:
    ...
    if 'tour' in url.lower() and '/event/' in url.lower():
        ...
        if is_event_page:
            ...
            is_button_clicked = press_button(
                driver, By.CSS_SELECTOR, '.btn.btn-signup')
        ...
        if '/activityinfo/details/' in url.lower():
            ...
            if is_event_page:
                if config_dict["date_auto_select"]["enable"]:
                    ...
                    is_date_assign_by_bot = ibon_date_auto_select(
                        driver, config_dict)
    ...
    if not is_match_target_feature:
        ...
        if is_event_page:
            ...
            ibon_dict["fail_list"] = ibon_verification_question(
        ...
        if is_event_page:
            if config_dict["area_auto_select"]["enable"]:
                ...
                if 'PRODUCT_ID=' in url.upper():
                    # step 1: select area.
                    is_match_target_feature = True
                    is_price_assign_by_bot = ibon_performance(
                        driver, config_dict)
                    # print("is_price_assign_by_bot:", is_price_assign_by_bot)
                    if not is_price_assign_by_bot:
                ...
                if is_do_ibon_performance_with_ticket_number:
                    if config_dict["advanced"]["disable_adjacent_seat"]:
                        is_finish_checkbox_click = ibon_allow_not_adjacent_seat(
                            driver, config_dict)
                    ...
                    if config_dict["ocr_captcha"]["enable"]:
                        ...
                        # PS: need set cookies once, if user change domain.
                        if not Captcha_Browser is None:
                            Captcha_Browser.set_domain(
                                domain_name, captcha_url=captcha_url)

                        is_captcha_sent = ibon_captcha(
                            driver, config_dict, ocr, Captcha_Browser, model_name)
            ...
            if is_event_page:
                ...
                if is_finish_checkbox_click:
                    ...
                    if not is_name_based:
                        is_button_clicked = press_button(
                            driver, By.CSS_SELECTOR, 'a.btn.btn-pink.continue')
...
def hkticketing_accept_cookie(driver):
    ...
    try:
        accept_all_cookies_btn = driver.find_element(
            By.CSS_SELECTOR, '#closepolicy_new')
    except Exception as exc:
        if show_debug_message:
...
def hkticketing_date_password_input(driver, config_dict, fail_list):
    ...
    try:
        ...
        el_password_input = driver.find_element(
            By.CSS_SELECTOR, my_css_selector)
    except Exception as exc:
        pass
...
def hkticketing_date_auto_select(driver, config_dict, fail_list):
    ...
    formated_area_list = None
    is_page_ready = True
    is_date_assigned, is_page_ready, formated_area_list = hkticketing_date_assign(
        driver, config_dict)
    ...
    is_auto_submit = True
    is_password_sent = False
    is_password_appear, fail_list = hkticketing_date_password_input(
        driver, config_dict, fail_list)
    ...
    if is_auto_submit:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, '#buyButton > input')
        if show_debug_message:
...
def hkticketing_next_button_press(driver):
    ...
    el_btn = None
    try:
        el_btn = driver.find_element(
            By.CSS_SELECTOR, '#continueBar > div.chooseTicketsOfferDiv > button')
    except Exception as exc:
        print("find next button fail...")
        print(exc)
...
def hkticketing_hide_tickets_blocks(driver):
    ...
    try:
        ...
        actionBlock_divs = driver.find_elements(
            By.CSS_SELECTOR, selector_string)
    ...
    try:
        ...
        detailModuleCopy_divs = driver.find_elements(
            By.CSS_SELECTOR, selector_string)
        if not detailModuleCopy_divs is None:
            driver.execute_script(
                "arguments[0].innerHTML='';", detailModuleCopy_divs)
    except Exception as exc:
        pass
    ...
    try:
        ...
        mapWrapper_divs = driver.find_elements(
            By.CSS_SELECTOR, selector_string)
        if not mapWrapper_divs is None:
            driver.execute_script(
                "arguments[0].innerHTML='';", mapWrapper_divs)
    except Exception as exc:
        pass
...
def hkticketing_performance(driver, config_dict, domain_name):
    ...
    if auto_fill_ticket_number:
        ...
        if len(area_keyword) > 0:
            ...
            for area_keyword_item in area_keyword_array:
                is_need_refresh, is_price_assign_by_bot = hkticketing_area_auto_select(
                    driver, config_dict, area_keyword_item)
                if not is_need_refresh:
                    break
                else:
        ...
        # choose ticket.
        is_ticket_number_assigned = hkticketing_ticket_number_auto_select(
            driver, config_dict)
        if show_debug_message:
            print("is_ticket_number_assigned:", is_ticket_number_assigned)
        ...
        if not 'galaxymacau.com' in domain_name:
            ...
            is_delivery_option_assigned = False
            if is_ticket_number_assigned:
                is_delivery_option_assigned = hkticketing_ticket_delivery_option(
                    driver)
            if show_debug_message:
                print("is_delivery_option_assigned:",
                      is_delivery_option_assigned)
...
def hkticketing_url_redirect(driver, url, config_dict):
    is_redirected = False
    redirect_url_list = ['queue.hkticketing.com/hotshow.html', '.com/detection.aspx?rt=', '/busy_galaxy.'
                         ]
    for idx in range(20):
    ...
    if url == 'https://entry-hotshow.hkticketing.com/':
        content_redirect_string_list = [
            'Access denied (403)', 'Current session has been terminated']
        is_need_refresh = False
        html_body = None
        try:
            html_body = driver.page_source
...
def hkticketing_content_refresh(driver, url, config_dict):
    ...
    check_url_list = [".com/default.aspx", ".com/shows/show.aspx?sh=", ".com/detection.aspx", "/entry-hotshow.", ".com/_Incapsula_Resource?"
                      ]
    ...
    check_full_url_list = ["https://premier.hkticketing.com/", "https://www.ticketing.galaxymacau.com/", "https://ticketing.galaxymacau.com/", "https://ticketing.galaxymacau.com/default.aspx"
                           ]
    for current_url in check_full_url_list:
        if current_url == url:
            is_check_access_deined = True

    content_retry_string_list = ["Access Denied", "Service Unavailable", "The service is unavailable", "HTTP Error 500", "HTTP Error 503", "504 Gateway Time-out", "502 Bad Gateway", "An error occurred while processing your request", "The network path was not found", "Could not open a connection to SQL Server", "Hi fans, you’re in the queue to", "We will check for the next available purchase slot", "please stay on this page and do not refresh", "Please be patient and wait a few minutes before trying again", "Server Error in '/' Application", "The target principal name is incorrect", "Cannot generate SSPI context", "System.Data.SqlClient.Sql", "System.ComponentModel.Win32Exception", "Access Denied", "Your attempt to access the web site has been blocked by", "This request was blocked by"
                                 ]
...
def hkticketing_travel_iframe(driver, config_dict):
    ...
    for iframe in iframes:
        ...
        try:
            ...
            is_redirected = hkticketing_content_refresh(
                driver, iframe_url, config_dict)
        except Exception as exc:
            pass
...
def softix_powerweb_main(driver, url, config_dict):
    ...
    if is_hkticketing_sign_in_page:
        ...
        if hkticketing_password == "":
            hkticketing_password = util.decryptMe(
                config_dict["advanced"]["hkticketing_password"])
        if len(hkticketing_account) > 4:
            hkticketing_login(driver, hkticketing_account,
            ...
            if is_event_page:
                if config_dict["date_auto_select"]["enable"]:
                    if not hkticketing_dict["is_date_submiting"]:
                        hkticketing_dict["is_date_submiting"], hkticketing_dict["fail_list"] = hkticketing_date_auto_select(
                            driver, config_dict, hkticketing_dict["fail_list"])
                        pass
                    else:
...
def khan_go_buy_redirect(driver, domain_name):
    ...
    if 'kham.com' in domain_name:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, 'div#content > p > a > button[onclick].red')
    if 'ticket.com' in domain_name:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, 'div.row > div > a.btn.btn-order.btn-block')
    if 'udnfunlife.com' in domain_name:
        # udn 快速訂購
        my_css_selector = 'button[name="fastBuy"]'
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, my_css_selector)
        if not is_button_clicked:
            is_button_clicked = press_button(
                driver, By.CSS_SELECTOR, '#buttonBuy')
    return is_button_clicked
...
def kham_product(driver, domain_name, config_dict):
    ...
    is_date_assign_by_bot = hkam_date_auto_select(
        driver, domain_name, config_dict)

    if not is_date_assign_by_bot:
        # click not on sale now.
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR, 'div.ui-dialog-buttonset > button.ui-button')
        pass
                        ...
                        if config_dict["ticket_number"] > 1:
                            ...
                            if maybe_ticket_count.isdigit():
                                if "<td" in row_html:
                                    ...
                                    if len(td_array) > 0:
                                        td_target = "<td" + \
                                            td_array[len(td_array)-1]
                                        ticket_count_text = util.remove_html_tags(
                                            td_target)
                                        ...
                                        if ticket_count_text.isdigit():
                                            if int(ticket_count_text) < config_dict["ticket_number"]:
                                                if show_debug_message:
                                                    print(
                                                        "skip this row, because ticket_count available only:", ticket_count_text)
                                                # skip this row.
                                                row_text = ""
...
def kham_switch_to_auto_seat(driver):
    ...
    try:
        ...
        btn_switch_to_auto_seat = driver.find_element(
            By.CSS_SELECTOR, my_css_selector)
    except Exception as exc:
    ...
    if not btn_switch_to_auto_seat is None:
        ...
        if button_class_string == "":
            try:
                ...
                try:
                    driver.execute_script(
                        "arguments[0].click();", btn_switch_to_auto_seat)
                    ret = True
                except Exception as exc:
                    pass
...
def kham_performance(driver, config_dict, ocr, Captcha_Browser, domain_name, model_name):
    ...
    if auto_fill_ticket_number:
        ...
        if len(area_keyword) > 0:
            ...
            for area_keyword_item in area_keyword_array:
                is_need_refresh, is_price_assign_by_bot = kham_area_auto_select(
                    driver, domain_name, config_dict, area_keyword_item)
                if not is_need_refresh:
                    break
                else:
                    print("is_need_refresh for keyword:", area_keyword_item)
    ...
    if not form_verifyCode is None:
        if len(answer) > 0:
            ...
            if inputed_value == "驗證碼":
                ...
                if len(inputed_value) > 0:
                    print("captcha text inputed:", inputed_value,
...
def kham_auto_ocr(driver, config_dict, ocr, away_from_keyboard_enable, previous_answer, Captcha_Browser, ocr_captcha_image_source, model_name):
    ...
    if not ocr_answer is None:
        ...
        print("ocr_answer:", ocr_answer)
        if len(ocr_answer) == 4:
            who_care_var = kham_keyin_captcha_code(
                driver, answer=ocr_answer, auto_submit=away_from_keyboard_enable)
        else:
            if not away_from_keyboard_enable:
...
def kham_captcha(driver, config_dict, ocr, Captcha_Browser, model_name):
    ...
    for redo_ocr in range(999):
        is_need_redo_ocr, previous_answer, is_form_sumbited = kham_auto_ocr(
            driver, config_dict, ocr, away_from_keyboard_enable, previous_answer, Captcha_Browser, ocr_captcha_image_source, model_name)
...
def kham_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    home_url_list = ['https://kham.com.tw/', 'https://kham.com.tw/application/utk01/utk0101_.aspx',
                     ...
                     'https://kham.com.tw/application/utk02/utk0201_040.aspx'
                     ]
    for each_url in home_url_list:
        if each_url == url.lower():
    ...
    if 'udnfunlife' in domain_name:
        ...
        if 'utk0203_.aspx?product_id=' in url.lower():
            ...
            try:
                ticket_number_text = driver.find_element(
                    By.CSS_SELECTOR, select_query)
            except Exception as exc:
                pass

            if not ticket_number_text is None:
                # layout format #1
                is_ticket_number_assigned = assign_text(driver, By.CSS_SELECTOR, select_query, str(
                    config_dict["ticket_number"]), overwrite_when="0")
                if is_ticket_number_assigned:
                    is_button_clicked = press_button(
                        driver, By.CSS_SELECTOR, '#buttonNext')
            else:
                # layout format #2
                date_auto_select_enable = config_dict["date_auto_select"]["enable"]
                if date_auto_select_enable:
        ...
        if 'utk0222_02.aspx?product_id=' in url.lower():
            ...
            if len(model_name) > 7:
                model_name = model_name[:7]
            is_price_assign_by_bot, is_captcha_sent = kham_performance(
                driver, config_dict, ocr, Captcha_Browser, domain_name, model_name)

            is_ticket_number_sent = assign_text(driver, By.CSS_SELECTOR, 'input#QRY2', str(
                config_dict["ticket_number"]), overwrite_when="0")
            if is_ticket_number_sent:
                is_fastbuy_pressed = press_button(
                    driver, By.CSS_SELECTOR, 'input#f_btn')
        ...
        if '.aspx?performance_id=' in url.lower() and 'product_id=' in url.lower():
            ...
            if config_dict["ocr_captcha"]["enable"]:
                is_reset_password_text = kham_check_captcha_text_error(
                    driver, config_dict)
            ...
            if config_dict["area_auto_select"]["enable"]:
                ...
                if config_dict["advanced"]["disable_adjacent_seat"]:
                    if "ticket.com.tw" in url:
                        is_finish_checkbox_click = ticket_allow_not_adjacent_seat(
                            driver, config_dict)
                    if "kham.com.tw" in url:
                        is_finish_checkbox_click = kham_allow_not_adjacent_seat(
                            driver, config_dict)

                if show_debug_message:
                    print("is_ticket_number_assigned:",
        ...
        if '/utk13/utk1306_.aspx' in url.lower():
            ...
            if config_dict["ocr_captcha"]["enable"]:
                ...
                if ticket_password == "":
                    ticket_password = util.decryptMe(
...
def ticketplus_date_auto_select(driver, config_dict):
    ...
    try:
        area_list = driver.find_elements(
            By.CSS_SELECTOR, 'div#buyTicket > div.sesstion-item > div.row')
        if not area_list is None:
            area_list_count = len(area_list)
            if area_list_count == 0:
    ...
    if is_vue_ready:
        ...
        if not target_area is None:
            target_button = None
            try:
                target_button = target_area.find_element(
...
def ticketplus_assign_ticket_number(target_area, config_dict):
    ...
    try:
        ...
        ticket_number_div = target_area.find_element(
            By.CSS_SELECTOR, my_css_selector)
    except Exception as exc:
        if show_debug_message:
        ...
        if len(ticket_number_text) > 0:
            ...
            if ticket_number_text_int < ticket_number:
                ...
                try:
                    ...
                    ticket_number_plus = target_area.find_element(
                        By.CSS_SELECTOR, my_css_selector)
                except Exception as exc:
                    if show_debug_message:
                ...
                if not ticket_number_plus is None:
                    ...
                    for i in range(add_count):
                        ...
                        try:
                            ...
                            ticket_number_text = ticket_number_div.text
                            if len(ticket_number_text) > 0:
                                ticket_number_text_int = int(
                                    ticket_number_text)
                                if show_debug_message:
                                    print("ticket_number_text_int:",
                                          ticket_number_text_int)
                                if ticket_number_text_int >= ticket_number:
                                    print("match target ticket count (now/target):",
                                          ticket_number_text_int, ticket_number)
                                    break
                        except Exception as exc:
                            pass
            else:
    ...
    try:
        ...
        if current_layout_style == 1:
            if len(area_list) == 0:
                ...
                price_group_list = driver.find_elements(
                    By.CSS_SELECTOR, my_css_selector)
    ...
    if not is_reset_query:
        if not target_area is None:
            for retry_index in range(2):
                ...
                is_price_assign_by_bot = ticketplus_assign_ticket_number(
                    target_area, config_dict)
                if is_price_assign_by_bot:
                    break
...
def ticketplus_order(driver, config_dict, ocr, Captcha_Browser, ticketplus_dict):
    ...
    if is_button_disabled:
        ...
        # print("is_price_assign_by_bot", is_price_assign_by_bot)
        if is_price_assign_by_bot:
            is_answer_sent, ticketplus_dict["fail_list"], is_question_popup = ticketplus_order_exclusive_code(
                driver, config_dict, ticketplus_dict["fail_list"])

            auto_submit = True
            if auto_submit:
                my_css_selector = "div.order-footer > div.container > div.row > div > button.nextBtn"
                is_form_sumbited = press_button(
                    driver, By.CSS_SELECTOR, my_css_selector)
...
def ticketplus_order_ocr(driver, config_dict, ocr, Captcha_Browser):
    ...
    last_url, is_quit_bot = get_current_url(driver)
    for redo_ocr in range(19):
        is_need_redo_ocr, previous_answer, is_form_sumbited = ticketplus_auto_ocr(
            driver, config_dict, ocr, previous_answer, Captcha_Browser)
...
def ticketplus_auto_ocr(driver, config_dict, ocr, previous_answer, Captcha_Browser):
    ...
    # check ocr inputed.
    is_verifyCode_editing, is_form_sumbited = ticketplus_keyin_captcha_code(
        driver)
...
svgToPng(svg, (imgData) => {
  ...
  callback(imgData);
    ...
    if not ocr_answer is None:
        ...
        print("ocr_answer:", ocr_answer)
        if len(ocr_answer) == 4:
            who_care_var, is_form_sumbited = ticketplus_keyin_captcha_code(
                driver, answer=ocr_answer, auto_submit=away_from_keyboard_enable)
        else:
            if not away_from_keyboard_enable:
                ...
                if previous_answer != ocr_answer:
                    ...
                    is_refresh_button_pressed = press_button(
                        driver, By.CSS_SELECTOR, my_css_selector)
                    ...
                    if len(action_btns) >= 4:
                        ...
                        if is_need_popup_modal:
                            ...
                            try:
                                driver.set_script_timeout(1)
                                driver.execute_script(
                                    "arguments[0].click();", action_btns[3])
                            except Exception as exc:
                                # print(exc)
                                pass

            is_account_sent, is_password_sent = ticketplus_account_sign_in(
                driver, config_dict)
            ...
            if len(inputed_value) > 0:
                ...
                if len(config_dict["advanced"]["ticketplus_account"]) == 0:
                    try:
                        # solution 1: js.
                        driver.execute_script(
                            "if(!(document.activeElement === arguments[0])){arguments[0].focus();}", form_account)
                        # solution 2: selenium.
                        # form_account.click()
                ...
                if el_pass.is_enabled():
                    ...
                    if not inputed_text is None:
                        ...
                        if ticketplus_password == "":
                            ticketplus_password = util.decryptMe(
...
def ticketplus_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    if '/order/' in url.lower():
        ...
        if is_event_page:
            ...
            if not is_reloading:
                ticketplus_dict = ticketplus_order(
                    driver, config_dict, ocr, Captcha_Browser, ticketplus_dict)
...
def facebook_main(driver, config_dict):
    ...
    if facebook_password == "":
        facebook_password = util.decryptMe(
...
def get_current_url(driver):
    ...
    try:
        ...
        exit_bot_error_strings = ['Max retries exceeded', 'chrome not reachable', 'unable to connect to renderer', 'failed to check if window was closed', 'Failed to establish a new connection', 'Connection refused', 'disconnected', 'without establishing a connection', 'web view not found', 'invalid session id'
                                  ]
...
def main(args):
    ...
    try:
        if config_dict["ocr_captcha"]["enable"]:
            ocr = ddddocr.DdddOcr(
                show_ad=False, beta=config_dict["ocr_captcha"]["beta"])
